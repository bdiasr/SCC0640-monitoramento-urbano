...
SELECT  HP.ONIBUS, HP.Latitude AS Lat_Ponto, HP.Longitude AS Lon_Ponto, ABS(24*60*EXTRACT(Hour FROM HP.Hora)+60*EXTRACT(MINUTE FROM HP.hora)+EXTRACT(SECOND FROM HP.hora)-(24*60*EXTRACT(Hour FROM SYSTIMESTAMP)+60*EXTRACT(MINUTE FROM SYSTIMESTAMP)+EXTRACT(SECOND FROM SYSTIMESTAMP))) AS AGR_MINUS_PADRAO , LC.Latitude AS Lat_Posicao, LC.Longitude AS Lon_Posicao,  ABS((24*60*EXTRACT(Hour FROM LC.DataHora)+60*EXTRACT(MINUTE FROM LC.Datahora)+EXTRACT(SECOND FROM LC.Datahora))-(24*60*EXTRACT(Hour FROM SYSTIMESTAMP)+60*EXTRACT(MINUTE FROM SYSTIMESTAMP)+EXTRACT(SECOND FROM SYSTIMESTAMP))) AS AGR_MINUS_LOCAL 
...
FROM (SELECT A1.Nome, B1.CPF, B1.placa, B1.PontoEntrada 
      From Pessoa A1 
      JOIN TURNO_MOTORISTA B1 
      ON A1.CPF = B1.CPF) M, 
     (SELECT A2.Nome, B2.CPF, B2.placa, B2.PontoEntrada 
      From Pessoa A2 
      JOIN TURNO_COBRADOR B2 
      ON A2.CPF = B2.CPF) C, LOCALIZACAO L
...
JOIN (SELECT PO.Latitude, PO.Longitude, PL.Codigo, PL.Ordenacao
...
MINUS
SELECT Endereco 
FROM (SELECT A.Endereco,B.CNPJ 
      FROM GARAGEM A, (SELECT CNPJ 
                       FROM EMPRESA 
                       WHERE CNPJ = '10.987.654/0001-32' OR CNPJ = '12.345.678/0001-91') B
      MINUS 
